...
for item in parsed_data:
    ...
    if norm_c == "k-water":
        norm_c = "한국수자원공사"
    company_map[norm_c].append(item)

duplicates_norm = {k: v for k, v in company_map.items() if len(v) > 1}
    ...
    for pd, pg, pn, pc in parsed_data:
        if pd == d:
            ...
            if pn == '유태수' and pd == '4/6':
                note += " (6주차(4/13) 게시판에 등록했으나 4/6으로 기재함)"
...
for row in ws.iter_rows(min_row=2, max_row=ws.max_row, min_col=2, max_col=3):
    for cell in row:
        if cell.value and ("중복선정" in cell.value or "게시판에 등록했으나" in cell.value):
            cell.font = red_font
